    ...
    <div class="container">
      <div class="row justify-content-center">
        <div class="col-lg-9 reveal">
          ...
          <p>{{ __('Oudaa is used by community committees (administrators) to keep records about residents, fees, funds, payments, projects and expenses. This policy covers two kinds of people:') }}</p>
          <ul class="list-check mb-4">
            <li><i class="bi bi-check"></i><div><strong class="text-navy">{{ __('Committee members') }}</strong> {{ __('— people who sign up, create a platform, and log in to manage it.') }}</div></li>
            <li><i class="bi bi-check"></i><div><strong class="text-navy">{{ __('Residents') }}</strong> {{ __('— people whose records a committee enters into the platform. Residents do not create their own accounts or log in; their information is entered and managed by the committee.') }}</div></li>
          </ul>

          <h3 class="mt-5 mb-3">{{ __('2. What we collect') }}</h3>
          <p><strong class="text-navy">{{ __('From committee members (account holders):') }}</strong> {{ __('name, email address, password (stored hashed, never in plain text), and the community/platform details provided during signup (community name, chosen subdomain, community type).') }}</p>
          ...
          <ul class="list-check mb-4">
            <li><i class="bi bi-check"></i><div>{{ __('To create and operate each community\'s platform.') }}</div></li>
          ...
          <p>{{ __('We do not sell resident or committee data, and we do not use it for advertising.') }}</p>

          <h3 class="mt-5 mb-3">{{ __('5. Who can see your data') }}</h3>
          <p>{{ __('Resident and financial records are visible only to the committee members of that specific platform, according to whatever access each member has been given. Oudaa\'s operators may access underlying data only when necessary to provide support, maintain the platform, or comply with the law.') }}</p>

          <h3 class="mt-5 mb-3">{{ __('6. Data retention') }}</h3>
          <p>{{ __('Records — including residents, fees, funds, payments, projects and expenses — are kept for as long as a community\'s platform is active. Deactivating a resident or retiring a fee does not delete its history; committees can archive rather than permanently delete records so past activity stays auditable.') }}</p>

          <h3 class="mt-5 mb-3">{{ __('7. Your rights') }}</h3>
          <p>{{ __('If you are a resident and want to know what information a committee has recorded about you, or want it corrected, please contact that community\'s committee directly, as they control their own platform\'s data. If you are a committee member and want your account or platform data removed, or have any other request, contact us using the details below.') }}</p>

          <h3 class="mt-5 mb-3">{{ __('8. Security') }}</h3>
          <p>{{ __('We use reasonable technical and organizational measures to protect data — including application-level access controls that isolate each community\'s records and hashed passwords — but no system is completely secure, and we cannot guarantee absolute security.') }}</p>

          <h3 class="mt-5 mb-3">{{ __('9. Changes to this policy') }}</h3>
          <p>{{ __('We may update this policy as Oudaa evolves. Material changes will be reflected by updating the "Last updated" date above.') }}</p>

          <h3 class="mt-5 mb-3">{{ __('10. Governing law') }}</h3>
          <p>{{ __('This policy is governed by the laws of Ethiopia.') }}</p>

          <h3 class="mt-5 mb-3">{{ __('11. Contact us') }}</h3>
          <p>{{ __('Questions about this policy or your data can be sent to') }} <a href="mailto:m7020322@gmail.com" class="text-primary-custom fw-semibold text-decoration-none">m7020322@gmail.com</a> {{ __('or') }} <a href="tel:+251973069687" class="text-primary-custom fw-semibold text-decoration-none">+251 973 069 687</a>.</p>
