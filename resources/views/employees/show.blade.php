...
<div class="panel">
    ...
    <div class="panel-body">
        <div class="form-grid">
            <div><span class="muted">{{ __('ID Number') }}</span><br>{{ $employee->id_number }}</div>
            <div><span class="muted">{{ __('Phone') }}</span><br>{{ $employee->phone ?: '—' }}</div>
            <div><span class="muted">{{ __('Payment Date') }}</span><br>{{ $employee->payment_date ? $employee->payment_date->format('jS \o\f each month') : '—' }}</div>
...
<div class="panel">
    <div class="panel-head"><h2>{{ __('Salary Payments') }}</h2></div>
    <div class="panel-body" style="padding:0;">
        @if ($employee->expenses->isEmpty())
            <div class="empty">No salary payments logged yet.</div>
        @else
            <table>
                <thead><tr><th>{{ __('Date') }}</th><th>{{ __('Amount') }}</th><th>{{ __('Note') }}</th></tr></thead>
                <tbody>
